...
import { ChevronDown, ChevronUp, Save, Upload, Trash2, Plus, X } from 'lucide-react';
import {
  fetchBrandDna,
  updateBrandSection,
  fetchBrandMedia,
...
type Section = 'overview' | 'visual_system' | 'photography_direction' | 'product' | 'ad_creative_style' | 'prompt_modifier';
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap gap-2">
        {values.map((v) => (
          <span
            key={v}
            className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-carbon-light text-sm text-gray-light"
          >
            {v}
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={add}
          className="px-3 py-1.5 bg-carbon-light rounded text-sm text-gray-light hover:text-white"
        >
          <Plus size={14} />
        </button>
      </div>
...
function MediaGallery({ type, label }: { type: string; label: string }) {
  const [files, setFiles] = useState<{ filename: string; url: string }[]>([]);
  const [uploading, setUploading] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    fetchBrandMedia(type).then(setFiles);
  }, [type]);
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const result = await uploadBrandMedia(type, file);
  ...
  const handleDelete = async (filename: string) => {
    await deleteBrandMedia(type, filename);
          ...
          {files.map((f) => (
            <div key={f.filename} className="group relative rounded-lg overflow-hidden bg-carbon aspect-square">
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                <p className="text-xs text-white text-center px-2 truncate w-full">{f.filename}</p>
                <button
    ...
    <div className="max-w-3xl mx-auto space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-mid mt-1">
          Manage your brand identity, visual system, and media assets.
        </p>
      </div>
      ...
      {/* Visual System */}
      <SectionCard
        title={SECTION_LABELS.visual_system}
        onSave={() => save('visual_system')}
        saving={saving === 'visual_system'}
      >
      ...
      <SectionCard
        title={SECTION_LABELS.photography_direction}
        onSave={() => save('photography_direction')}
        saving={saving === 'photography_direction'}
      >
      ...
      {/* Product */}
      <SectionCard
        title={SECTION_LABELS.product}
        onSave={() => save('product')}
        saving={saving === 'product'}
      >
        <TextField label="Product Type" value={get('product', 'type')} onChange={(v) => set('product', 'type', v)} />
        <TextField label="Description" value={get('product', 'description')} onChange={(v) => set('product', 'description', v)} multiline />
        <TextField label="Price" value={get('product', 'price')} onChange={(v) => set('product', 'price', v)} />
        <TextField label="Delivery Platform" value={get('product', 'delivery_platform')} onChange={(v) => set('product', 'delivery_platform', v)} />
        <div className="space-y-1">
          <label className="text-xs font-medium text-gray-mid uppercase tracking-wider">Distinctive Features</label>
          <TagList values={getList('product', 'distinctive_features')} onChange={(v) => set('product', 'distinctive_features', v)} />
        </div>
        <TextField label="Product Ecosystem" value={get('product', 'ecosystem')} onChange={(v) => set('product', 'ecosystem', v)} multiline />
      </SectionCard>

      {/* Ad Creative Style */}
      <SectionCard
        title={SECTION_LABELS.ad_creative_style}
        onSave={() => save('ad_creative_style')}
        saving={saving === 'ad_creative_style'}
      >
      ...
      <SectionCard
        title={SECTION_LABELS.prompt_modifier}
        onSave={() => save('prompt_modifier')}
        saving={saving === 'prompt_modifier'}
      >
        ...
        <textarea
          ...
          onChange={(e) =>
            setDna((prev) => ({ ...prev!, image_prompt_modifier: e.target.value }))
          }
          rows={5}
      ...
      {/* Media */}
      <div>
        <h3 className="text-base font-semibold text-white mb-3">Media Assets</h3>
        <div className="space-y-4">
          <MediaGallery type="product-images" label="Product Images" />
          <MediaGallery type="reference-images" label="Reference Images" />
        </div>
      </div>
    </div>
  );
